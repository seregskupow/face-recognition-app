...
import AvengersModel from '../images/models/background.glb';
...
export default class Background{
  ...
  init() {
    ...
    if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
      ...
      document.body.addEventListener('mousemove',_.throttle(this.onMouseMove, 100));
     }
  ...
  handleOrientation(event){
    const yMIN = -0.02, yMAX=0.1,xMIN=-0.3,xMAX=0.3;
    let rotateX=event.alpha, 
    rotateY=event.beta;
  ...
  onMouseMove(event) {
    ...
    let isMobile=false;
      gsap.to(this.mousePos, 4, {
          x: Math.min(Math.max((event.clientX / window.innerWidth) * 2 - 1, xMIN), xMAX),
          y: -Math.min(Math.max(((event.clientY / window.innerHeight) * 2 - 1), yMIN), yMAX),
          onUpdate:  ()=> {
              this.camera.lookAt(this.mousePos);
          }
      });
