    ...
    return new Promise(async function (resolve, reject) {
      ...
      try {      
        ...
        for (let i = 0; i < names.length; i++) {
          ...
          let el = await page.$(
            'article>section:nth-child(1)>div>a>img'
          );
          if (!el) {
            urlToParse =
              url +
          ...
          let birthday = await getInnerText(page,'article>section:nth-child(1)>div>div>div>p:nth-of-type(3)');
          const birthPlace = await getInnerText(page,'article>section:nth-child(1)>div>div>div>p:nth-of-type(4)');
          const biography = await getInnerText(page,'article>section:nth-child(1)>div>div>div>p:nth-of-type(5)');
          ...
          if (featured.length > 0) {
            for (let i = 1; i <= featured.length; i++) {
              ...
              let filmName = (await getInnerText(page, `.celebrity-highest__list>li:nth-child(${i})>div>p>a`))
                .trim()
                .replace(/\s|\n|\s\s/g, "_")
                .split("<")[0]
                .split("_")
